        ...
        timestamps
        {
            ...
            gitlabCommitStatus(name: stageName)
            {
                    ...
                    currentBuild.result == 'FAILURE')
                {
                    ...
                    // Stop the entire pipeline and report an error if the build
                    // is set to FAILURE.  No sense proceeding when we know now
                    // there is a problem to fix.  This might need to change
                    // if/when the pipeline becomes parallelized.
