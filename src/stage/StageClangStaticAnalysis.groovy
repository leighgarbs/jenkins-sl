...
class StageClangStaticAnalysis extends Stage
{
                             ...
                             boolean runOnWindows = false)
    {
        ...
        super(wfc, 'CLANG STATIC ANALYSIS', cleanWorkspace, runOnLinux, runOnWindows)
    }

    boolean runLinux()
    {
        // This writes clang.debug.out to the workspace so the recordIssues step below can
        // ingest it.
        def returnCode = wfc.runResourceScript(wfc, 'linux/stageClangStaticAnalysis')
        ...
        tools: [wfc.clang(name: 'Clang Static Analysis', pattern: 'clang.debug.out')]
